...
const VideoRequest = () => {
  ...
  const [form, setForm] = useState({ image: "", name: "", video: "", mobile: "", orderDate: "", status: "" });
  ...
  const handleOpen = () => {
    setEditId(null);
    setForm({ image: "", name: "", video: "", mobile: "", orderDate: "", status: "" });
    setOpen(true);
  };
    ...
    <div style={{ padding: "20px", marginBottom: "4px", marginTop: "4px" }}>
      ...
      <TableContainer component={Paper}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell sx={{ backgroundColor: '#e3f2fd' }}>Thumbnail</TableCell>
              <TableCell sx={{ backgroundColor: '#e3f2fd' }}>
                <Box display="flex" alignItems="center" gap={0.5}>
                  Name
                  <IconButton onClick={() => handleSort("name")} sx={{ padding: '2px', fontSize: '16px' }} size="small">
                    {sortConfig.key === "name" ? (
                      sortConfig.direction === "asc" ? <MdArrowUpward fontSize="inherit" /> : <MdArrowDownward fontSize="inherit" />
                    ) : <MdArrowDownward fontSize="inherit" style={{ opacity: 0.3 }} />}
                  </IconButton>
                </Box>
              </TableCell>
              ...
              <TableCell sx={{ backgroundColor: '#e3f2fd' }}>
                <Box display="flex" alignItems="center" gap={0.5}>
                  Order Date
                  <IconButton onClick={() => handleSort("orderDate")} sx={{ padding: '2px', fontSize: '16px' }} size="small">
            ...
            {filteredData.length > 0 ? (
              filteredData.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell><a href={item.video} target="_blank" rel="noopener noreferrer">Watch</a></TableCell>
                  <TableCell>{item.mobile}</TableCell>
                  <TableCell>{item.orderDate}</TableCell>
                  <TableCell>{item.status}</TableCell>
                  <TableCell>
                    <Button variant="outlined" color="primary" size="small" sx={{ mr: 1 }} onClick={() => handleEdit(item)}>Edit</Button>
                    <Button variant="outlined" color="error" size="small" onClick={() => handleDelete(item.id)}>Delete</Button>
                  </TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
                <TableCell colSpan={7} align="center">No matching records found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Box display="flex" justifyContent="center" alignItems="center" mt={2} gap={1} flexWrap="wrap">
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth PaperProps={{ sx: { boxShadow: '0px 8px 24px rgba(0, 0, 0, 0.2)', borderRadius: 2 } }}>
        <DialogTitle>{editId ? "Edit Video Request" : "Add New Video Request"}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
            <Button variant="outlined" component="label" sx={{ '&:hover': { backgroundColor: '#e0f0ff' } }}>
              Upload Image
            ...
            <TextField label="Card Name" name="name" value={form.name} onChange={handleChange} fullWidth />
            ...
            <TextField label="Order Date" name="orderDate" type="date" InputLabelProps={{ shrink: true }} value={form.orderDate} onChange={handleChange} fullWidth />
